...
const CreateAvailabilityScreen: React.FC = () => {
  ...
  const navigateToMainScreen = useCallback(() => {
    try {
      // Multiple navigation strategies to ensure we reach the main screen
      
      // Strategy 1: Reset navigation stack to tabs and go to meet tab
      navigation.dispatch(
        CommonActions.reset({
          index: 0,
          routes: [
      ...
      // Fallback strategy 2: Simple navigation to meet screen
      try {
        navigation.navigate('(tabs)', { screen: 'meet' });
      } catch (fallbackError) {
        // Fallback strategy 3: Go back
        navigation.goBack();
      }
    }
  }, [navigation]);
